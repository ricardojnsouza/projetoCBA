...
function enviarEmail($email, $token) {
    ...
    try {
        ...
        $mail->Username   = $GLOBALS['recuperacao_email_endereco']; //SMTP username
        $mail->Password   = $GLOBALS['recuperacao_email_senha'];    //SMTP password
        $mail->SMTPSecure = PHPMailer::ENCRYPTION_SMTPS;            //Enable implicit TLS encryption
        $mail->Port       = 465;                                    //TCP port to connect to; use 587 if you have set `SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS`

        //Recipients
        $mail->setFrom($GLOBALS['recuperacao_email_endereco'], 'Mailer');
        $mail->addAddress($email, $email);     //Add a recipient
        //$mail->addAddress('joe@example.net', 'Joe User');     //Add a recipient
        //$mail->addAddress('ellen@example.com');               //Name is optional
        $mail->addReplyTo($GLOBALS['recuperacao_email_endereco'], 'Informações');
