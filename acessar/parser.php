<?php
    ...
    // - validação: checa a autenticação do usuário
    $auth = false;
    if (count($criticas) === 0 && !($auth = login($post['user'], $post['pass']))) {
        $criticas[] = "<strong>Usuário</strong> ou <strong>Senha</strong> inválidas!";
        if (isBlockedUser($post['user'])) {
            $criticas[] = "<strong>Usuário</strong> bloqueado por 15 minutos!";
        }
    }
